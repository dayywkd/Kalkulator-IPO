...
export default function App() {
  ...
  // State Tema Dark/Light Mode (Default Dark)
  ...
  return (
    <div className="min-h-screen pb-16">
      ...
      <button
        ...
        className="fixed top-6 right-6 z-50 p-2.5 rounded-xl glass-panel hover:bg-bg-surface/80 active:scale-95 transition-all shadow-lg cursor-pointer"
        aria-label="Toggle Theme"
      >
        {isDarkMode ? (
          // Icon Matahari
          <svg className="w-5 h-5 text-accent-terracotta" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <header className="pt-8 pb-4 text-center max-w-7xl mx-auto px-4">
        <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-text-primary drop-shadow-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 mb-6 flex justify-center">
        <div className="flex bg-bg-surface/30 border border-border-custom rounded-xl p-1 shadow-inner backdrop-blur-sm">
          <button
          ...
          <>
            ...
            {hasActiveOrder && (
              <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-fade-in">
                ...
                <div className="glass-panel rounded-xl p-5 border-l-2 border-positive-ara shadow-sm">
                  <div className="text-xs font-semibold text-text-secondary uppercase tracking-wider">Potensi Profit Maksimal (Hari ke-1 ARA)</div>
                  <div className="flex items-baseline gap-2 mt-2">
                    <span className="text-2xl md:text-3xl font-bold font-mono text-positive-ara">
                  ...
                  <p className="text-xs text-text-secondary mt-2 leading-relaxed">
                    Proyeksi nilai total portofolio Anda akan menjadi <strong className="text-text-primary font-mono">{formatRupiah(totalOrder + totalAraPnL)}</strong> jika seluruh saham yang dipesan mengalami Auto Rejection Atas di hari pertama.
                  </p>
                </div>

                {/* Ringkasan Risiko ARB */}
                <div className="glass-panel rounded-xl p-5 border-l-2 border-negative-arb shadow-sm">
                  <div className="text-xs font-semibold text-text-secondary uppercase tracking-wider">Potensi Kerugian Maksimal (Hari ke-1 ARB)</div>
                  <div className="flex items-baseline gap-2 mt-2">
                    <span className="text-2xl md:text-3xl font-bold font-mono text-negative-arb">
                      {formatRupiah(totalArbPnL)}
                    </span>
                    <span className="text-xs font-bold font-mono bg-negative-arb/10 text-negative-arb px-2 py-0.5 rounded">
                      {totalArbPnLPercentage.toFixed(2)}%
                    </span>
                  </div>
                  <p className="text-xs text-text-secondary mt-2 leading-relaxed">
                    Proyeksi nilai total portofolio Anda akan menjadi <strong className="text-text-primary font-mono">{formatRupiah(totalOrder + totalArbPnL)}</strong> jika seluruh saham yang dipesan mengalami Auto Rejection Bawah di hari pertama.
                  </p>
                </div>
            ...
            <section className="flex flex-col gap-4">
              <div className="flex justify-between items-center px-1">
                <h2 className="text-lg md:text-xl font-bold text-text-primary tracking-wide">
                  Daftar Penawaran IPO Aktif
                </h2>
                {hasActiveOrder && (
                  <button 
                    onClick={() => setShowResetModal(true)}
                    className="text-xs font-bold text-negative-arb hover:text-negative-arb/85 bg-negative-arb/10 hover:bg-negative-arb/15 border border-negative-arb/20 py-1.5 px-3.5 rounded-lg active:scale-95 transition-all cursor-pointer"
                  >
                    Reset Pilihan
                  </button>
                )}
              ...
              {loading ? (
                <div className="glass-panel rounded-xl p-12 text-center text-text-secondary flex flex-col items-center justify-center gap-3">
                  <svg className="animate-spin h-8 w-8 text-accent-terracotta" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                  </svg>
                  <span className="font-semibold text-xs uppercase tracking-wider">Memuat data IPO saham...</span>
                </div>
              ) : (
                <>
      ...
      {showResetModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
          ...
          {/* Modal Container */}
          <div className="relative glass-panel max-w-xs w-full rounded-2xl border border-border-custom p-6 shadow-2xl">
            <div className="flex flex-col items-center text-center">
               {/* Warning Icon */}
              <div className="w-12 h-12 rounded-xl bg-negative-arb/10 border border-negative-arb/20 flex items-center justify-center mb-4">
                <svg className="w-6 h-6 text-negative-arb" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="flex w-full gap-3 mt-6">
                <button
                  onClick={() => setShowResetModal(false)}
                  className="flex-1 py-2.5 rounded-xl border border-border-custom text-xs font-semibold text-text-secondary hover:bg-bg-surface/50 active:scale-95 transition-all cursor-pointer"
                >
                  Batal
                </button>
                <button
                  onClick={confirmReset}
                  className="flex-1 py-2.5 rounded-xl bg-negative-arb hover:bg-negative-arb/90 text-xs font-semibold text-white active:scale-95 transition-all cursor-pointer"
                >
                  Ya, Reset
                </button>
              </div>
